...
class _CameraScreenState extends State<CameraScreen> {
  ...
  void initState() {
    ...
    // Lets the "Widget capture" scenarios push this screen's scoreband widget as a dynamic overlay.
    _studio.captureBand = ({bool advance = false}) async {
      if (advance && mounted) {
        setState(() {
          _homeScore += 1 + Random().nextInt(3);
          _matchTime += 1;
        });
      }
      return captureBoundaryPng(_scoreBandKey);
    };
  ...
  Future<void> _pushScoreband() async {
    final bytes = await captureBoundaryPng(_scoreBandKey, pixelRatio: 1.0);
    if (bytes == null) {
      debugPrint('[scoreband] widget not on screen yet — nothing captured');
      return;
    }
    try {
      debugPrint('[scoreband] captured ${bytes.length} bytes — sending to native');
